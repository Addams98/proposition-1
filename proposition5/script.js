// Proposition 5 — "Vision Magazine" script
(function () {
  'use strict';
  ...
  const onScroll = () => {
    if (navbar) navbar.style.boxShadow = window.scrollY > 10
      ? '0 2px 24px rgba(27,42,107,0.14)'
      : '0 1px 0 rgba(27,42,107,0.11)';
  };
  ...
  const pbio = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        ...
        if (fill) { const w = fill.getAttribute('data-width') || fill.style.width; fill.style.width = '0'; requestAnimationFrame(() => { fill.style.width = w; }); }
